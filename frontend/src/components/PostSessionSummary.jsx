...
const PostSessionSummary = ({ telemetryData, sessionId, onReset }) => {
  ...
  return (
    <div className="card" style={styles.container}>
      ...
      {signatureData && (
        <div className="print-only" style={{ display: 'none', marginTop: '2rem', padding: '1.5rem', border: '2px solid black', backgroundColor: '#fff', color: '#000' }}>
          <h3 style={{ borderBottom: '1px solid black', paddingBottom: '0.5rem', marginBottom: '1rem' }}>CRYPTOGRAPHIC EXPORT SIGNATURE</h3>
          <p style={{ fontWeight: 'bold', textTransform: 'uppercase', marginBottom: '1rem' }}>{signatureData.legal_disclaimer}</p>
          <div style={{ wordBreak: 'break-all', fontFamily: 'monospace', fontSize: '0.85rem' }}>
            <p><strong>Payload:</strong> {signatureData.data}</p>
            <p><strong>Signature:</strong> {signatureData.signature}</p>
          </div>
          <p style={{ marginTop: '1rem', fontStyle: 'italic', fontSize: '0.8rem' }}>Generated by Project SANKET Compliance Layer. This document has been cryptographically sealed by a two-officer authorization protocol.</p>
        </div>
      )}
      ...
      <div style={styles.footer} className="no-print">
        ...
        {showExportForm && !exportSuccess && (
          <div style={styles.exportForm} className="export-form">
